...
class BirdConfig(object):
  ...
  def save(self):
    # Write route announcements
    # Create/rewrite config file
    logging.info('Writing routes announcements for IPv%d to BIRD configs', self.version)
    try:
    ...
    try:
      c = open(self._filepath_routes, 'w')
    except:
      logging.exception('Cannot open %s for writing.', self._filepath)
      raise
    c.write("# Generated by ip-control at {}. Do not touch this file!\n".format(str(datetime.now())))
    # Put our IPs into the file
    for network in self._networks:
      c.write('route {} via "{}";\n'.format(network, self._get_interface(network)))
    c.close()
    # Reload our bird
    try:
